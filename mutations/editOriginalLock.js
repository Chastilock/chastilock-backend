...
async function editOriginalLock(inputs, models, req) {
    ...
    if(CreatedLockRecord == null || CreatedLockRecord.User_ID != req.Authenticated) {
        throw new UserInputError("Unable to find the lock to edit or you aren't the owner of it")
    }

    const OriginalLockRecord = await models.OriginalLockType.findOne({
        where: {
            Original_Deck_ID: CreatedLockRecord.OriginalLockType_ID
        }
    });

    if(OriginalLockRecord == null) {
    ...
    await OriginalLockRecord.save();

    await CreatedLockRecord.set({
    ...
    await CreatedLockRecord.save();
    return CreatedLockRecord;
}
module.exports = editOriginalLock;
