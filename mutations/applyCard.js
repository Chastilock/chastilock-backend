...
const { LoadedLock, Freeze, LoadedOriginalLock } = require("../models");
...
async function applyCard(inputs, models, req) {
  ...
  const deck = await LoadedOriginalLock.findByPk(lock.Original_Lock_Deck)

  if (deck.Chances_Remaining < 1) {
